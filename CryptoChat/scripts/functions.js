...
function updateCookie() {
    ...
    $.cookie("keptValues", JSON.stringify(values));
}
...
function onDocumentReady() {
    var keptValues = $.cookie("keptValues");
    ...
    var navControl = navControl || {
        challengeLink: function () {
            ...
            for (var i = 0; i < chatBuddies.length; i++) {
                if (chatBuddies[i].name == challUser) {
                    chatControl.endChat(challUser);
        ...
        regButton: function () {
            ...
            if (user.length < 3 || user.length > 30) {
                $("#error-message").text("Username must be between 3 and 30 symbols!");
                return;
            }
            var password = document.getElementById("password-tb").value;
            var repPassword = document.getElementById("rep-password-tb").value;
            if (password != repPassword) {
                $("#error-message").text("You have to type the same password!");
                return;
            }
            if (password.length < 8 || password.length > 30) {
                $("#error-message").text("Password must be between 8 and 30 symbols!");
                return;
            }
    ...
    var loginControl = loginControl || {
        loggedIn: function (data) {
            ...
            $("#logout-link").on("click", navControl.logoutLink);
            performRequest.get(rootUrl + "list-users/" + ID, ID, handleUsers.loadAllUsers, loginControl.logOut);
            message = setInterval(loginControl.getMessage, 500);
            updateCookie();    
        },
        getMessage: function () {
    ...
    var handleErrors = handleErrors || {
        regErr: function (error) {
            ...
            if (error.errorCode == "ERR_DUPLICATE") {
                ...
                $("#error-message").text("Invalid username. Use only letters, numbers and '.', '-', '_' for up to 30 symbols.");
            }
        },
